...
If you are running Hymir behind a proxy server, it is important to configure the webserver to set the `X-Forwared-For` and `X-Forwarded-Proto` headers, because they are used e.g. in rendering absolute URLs in `info.json` response content.
